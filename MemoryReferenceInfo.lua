...
local function GetTableName(cObject,strName)
...
local function GetTableName2(cObject,strName)
	local strType = type(cObject)
	if "table" == strType then
		return GetTableName(cObject,strName)
	end
	return strName
end
...
local function GetOriginalToStringResult(cObject)
	...
	if not cMt then
		return GetTableName2(cObject,tostring(cObject))
	end
	...
	return GetTableName2(cObject,strName)
end
...
local function CreateObjectReferenceInfoContainerFromFile(strFilePath)
	...
	for strLine in cFile:lines() do
		...
		if "--" ~= strHeader then
			...
			if strAddr then
				cRefInfo[strAddr] = strRefCount or 0
				cNameInfo[strAddr] = strName or 0
			end
		end
	end
...
local function CollectObjectReferenceInMemorySimple(strName, cObject, cDumpInfoContainer)
	if not cObject then
		return
	end
	...
	if "table" == strType then
		...
		-- strName = GetTableName(cObject,strName)
		...
		for k, v in pairs(cObject) do
			...
			if "table" == strKeyType then
				if not bWeakK then
					CollectObjectReferenceInMemorySimple(strName , k, cDumpInfoContainer)
				end

				if not bWeakV then
					CollectObjectReferenceInMemorySimple(strName , v, cDumpInfoContainer)
				end
		...
		-- Dump metatable.
		if cMt then
			CollectObjectReferenceInMemorySimple(strName, cMt, cDumpInfoContainer)
		end
	elseif "function" == strType then
		...
		for i = 1, nUpsNum do
			...
			if "table" == strUpValueType then
				CollectObjectReferenceInMemorySimple(strName , cUpValue, cDumpInfoContainer)
		...
		if getfenv then
			...
			if cEnv then
				CollectObjectReferenceInMemorySimple(strName , cEnv, cDumpInfoContainer)
			end
		end
...
local function CollectObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	if "table" == strType then
		...
		for k, v in pairs(cObject) do
			...
			elseif "thread" == strKeyType then
				if not bWeakK then
					CollectObjectReferenceInMemory(strName .. ".[t:k.th]", k, cDumpInfoContainer)
				end
			...
			elseif "userdata" == strKeyType then
				if not bWeakK then
					CollectObjectReferenceInMemory(strName .. ".[t:k.u]", k, cDumpInfoContainer)
				end
	...
	elseif "function" == strType then
		...
		for i = 1, nUpsNum do
			...
			--print(strUpName, cUpValue)
			if "table" == strUpValueType then
				CollectObjectReferenceInMemory(strName .. ".[ups:t:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			elseif "function" == strUpValueType then
				CollectObjectReferenceInMemory(strName .. ".[ups:f:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			elseif "thread" == strUpValueType then
				CollectObjectReferenceInMemory(strName .. ".[ups:th:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			elseif "userdata" == strUpValueType then
				CollectObjectReferenceInMemory(strName .. ".[ups:u:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			end
		end
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectObjectReferenceInMemory(strName ..".[f:ev]", cEnv, cDumpInfoContainer)
			end
		end
	elseif "thread" == strType then
		-- Add reference and name.
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectObjectReferenceInMemory(strName ..".[th:ev]", cEnv, cDumpInfoContainer)
			end
		end
		...
		local cMt = getmetatable(cObject)
		if cMt then
			CollectObjectReferenceInMemory(strName ..".[th:mt]", cMt, cDumpInfoContainer)
		end
	elseif "userdata" == strType then
		-- Add reference and name.
		cRefInfoContainer[cObject] = (cRefInfoContainer[cObject] and (cRefInfoContainer[cObject] + 1)) or 1
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectObjectReferenceInMemory(strName ..".[u:ev]", cEnv, cDumpInfoContainer)
			end
		end
		...
		local cMt = getmetatable(cObject)
		if cMt then
			CollectObjectReferenceInMemory(strName ..".[u:mt]", cMt, cDumpInfoContainer)
		end
    elseif "string" == strType then
        -- Add reference and name.
        cRefInfoContainer[cObject] = (cRefInfoContainer[cObject] and (cRefInfoContainer[cObject] + 1)) or 1
...
local function CollectSingleObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	if "table" == strType then
		...
		for k, v in pairs(cObject) do
			...
			elseif "thread" == strKeyType then
				if not bWeakK then
					CollectSingleObjectReferenceInMemory(strName .. ".[t:k.th]", k, cDumpInfoContainer)
				end
			...
			elseif "userdata" == strKeyType then
				if not bWeakK then
					CollectSingleObjectReferenceInMemory(strName .. ".[t:k.u]", k, cDumpInfoContainer)
				end
	...
	elseif "function" == strType then
		...
		for i = 1, nUpsNum do
			...
			elseif "thread" == strUpValueType then
				CollectSingleObjectReferenceInMemory(strName .. ".[ups.th:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			elseif "userdata" == strUpValueType then
				CollectSingleObjectReferenceInMemory(strName .. ".[ups.u:" .. strUpName .. "]", cUpValue, cDumpInfoContainer)
			end
		end
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectSingleObjectReferenceInMemory(strName ..".[f:mt]", cEnv, cDumpInfoContainer)
			end
		end
	elseif "thread" == strType then
		-- Check if the specified object.
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectSingleObjectReferenceInMemory(strName ..".[th:ev]", cEnv, cDumpInfoContainer)
			end
		end
		...
		local cMt = getmetatable(cObject)
		if cMt then
			CollectSingleObjectReferenceInMemory(strName ..".[th:mt]", cMt, cDumpInfoContainer)
		end
	elseif "userdata" == strType then
		-- Check if the specified object.
		if cExistTag[cObject] and (not cNameAllAlias[strName]) then
		...
		if getfenv then
			local cEnv = getfenv(cObject)
			if cEnv then
				CollectSingleObjectReferenceInMemory(strName ..".[u:ev]", cEnv, cDumpInfoContainer)
			end
		end
		...
		local cMt = getmetatable(cObject)
		if cMt then
			CollectSingleObjectReferenceInMemory(strName ..".[u:mt]", cMt, cDumpInfoContainer)
		end
    elseif "string" == strType then
        -- Check if the specified object.
        if cExistTag[cObject] and (not cNameAllAlias[strName]) then
...
local function format_out_string(objName, refPath, refNum, isSimple)
	if isSimple then
		-- local t = {objName,"\t","ry","\t","555\n"}
		-- return table.concat(t)
		return objName .. "\tryxx\t555\n"
		-- return objName .. "\n"
		-- return objName .."\t0\t0\n"
		-- return objName .."   0    1\n"
	else
		return objName .. "\t" .. refPath .. "\t" .. refNum .. "\n"
	end
end
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults,isSimple)
	-- Check results.
	...
	local strTable = {}
	local cOutputer = function (strContent,isIm)
		if isIm == nil then
			if #strTable < 10000 then
				table.insert(strTable,strContent)
				return
			end
		else
			if #strTable <= 0 then
		...
		strContent = table.concat(strTable)
		strTable = {}
				...
				if (i <= nMaxRescords) then
                    if "string" == type(v) then
                        ...
                        if ((not cDumpInfoResultsBase) and ((nil == nPattenBegin) or (nil == nPattenEnd))) then
							...
							cOutputer(format_out_string("string: \"" .. strRepString .. "\"", cNameInfo[v] , tostring(cRefInfo[v]),isSimple))
                            -- cOutputer("string: \"" .. strRepString .. "\"\t" .. cNameInfo[v] .. "\t" .. tostring(cRefInfo[v]) .. "\n")
                        else
							cOutputer(format_out_string(tostring(v) ,cNameInfo[v] ,tostring(cRefInfo[v]) ,isSimple))
                            -- cOutputer(tostring(v) .. "\t" .. cNameInfo[v] .. "\t" .. tostring(cRefInfo[v]) .. "\n")
                        end
                    else
						cOutputer(format_out_string(GetOriginalToStringResult(v),cNameInfo[v] ,tostring(cRefInfo[v]) ,isSimple))
...
local function DumpMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject,isSimple)
	-- Get time format string.
	local strDateTime = FormatDateTimeNow()
	...
	if isSimple then
		CollectObjectReferenceInMemorySimple(strRootObjectName, cRootObject, cDumpInfoContainer)
	else
	-- Collect memory info.
	...
	-- Dump the result.
	OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, nil, cDumpInfoContainer,isSimple)
end
